...
- **Structure controls in the config screen** (a "Structures" tab, driven by a per-mod `structures` block in `replace_vanilla.json`):
  ...
  - **Disable** - a per-structure toggle that stops any Moogs structure from generating, enforced by structure id at `tryGenerateStructure` (works for every placement type). `/locate` on a disabled structure reports it instead of searching. With an optional `structure_id` on `advanced_random_spread` / `conditional_concentric_rings`, a disabled structure also stops reporting placement positions, so it no longer influences neighbouring structures' exclusion zones or computes concentric rings.
