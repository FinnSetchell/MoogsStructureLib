...
public class MoogsStructuresCommon {
    ...
    public static void registerDatapackListener(final RegisterReloadListenerEvent event) {
        event.register(Identifier.fromNamespaceAndPath(MODID, "trial_spawner_config_manager"), TrialSpawnerConfigManager.INSTANCE);
        event.register(Identifier.fromNamespaceAndPath(MODID, "structure_manifests"), new StructureManifestReloadListener());
        // Without this the msl_pieces_spawn_counts data is never loaded, so per-piece spawn counts are inert.
        event.register(Identifier.fromNamespaceAndPath(MODID, "msl_pieces_spawn_counts"), StructurePieceCountsManager.STRUCTURE_PIECE_COUNTS_MANAGER);
    }
}
