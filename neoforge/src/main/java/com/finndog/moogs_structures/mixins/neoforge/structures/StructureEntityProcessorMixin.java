...
/**
 * NeoForge bridge for MSL's entity processors. NeoForge's structure placement invokes
 ...
 * descriptor (an overload, not an override), so without this NeoForge runs its no-op default and
 * MSL entity processors like {@code equip_armor_stand} never fire. This adds the missing override
 * and forwards to MSL's method. (Fabric does the equivalent via {@code EntityProcessorMixin} on
 * {@code placeEntities}.)
 */
...
public abstract class StructureEntityProcessorMixin {
    ...
    public StructureTemplate.StructureEntityInfo processEntity(LevelReader worldReader, BlockPos piecePos,
        ...
        // NeoForge gives no pivot, so reuse piecePos for the bottom-center arg (equip does not use it).
